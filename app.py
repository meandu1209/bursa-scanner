...
if st.button("Run Batch Scanner"):
    with st.spinner("Fetching data in batches..."):
        results = []
        chunk_size = 20  # Process 20 stocks at a time to avoid timeouts
        ...
        for i in range(0, len(tickers), chunk_size):
            ...
            for ticker in chunk:
                try:
                    # Safely handle single vs multi-ticker results from yfinance
        ...
        st.dataframe(
            ...
            column_config={"Matches Criteria": st.column_config.CheckboxColumn("Matches Criteria")}
        )